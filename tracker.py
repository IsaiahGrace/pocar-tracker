'''
  ...
  -
  Any updatates to this file will have to be cemented with:
...
USER_BUTTON = 21 
USER_LED = 20
ENABLE = 17 # Called DISABLE in docs. Powers the voltage regulator for the entire HAT
POWERKEY = 24 # M95 Module
STATUS = 23 # M95 Module
L96_RESET = 26 
GPS_FIX = 5
GEO_FENCE = 19
FORCE_ON = 13 # L96 pulling this high will force L96 out of backup mode. "$PMTK255,4*2F" will cause the L96 to go into backup mode until FOCE_ON goes high
PPS_PIN = 6 # L96 one pulse per second
L96_SOFT_RX = 22
L96_SOFT_TX = 27
...
fileLog = open("/home/pi/logs/tracker_log.txt","w+") 
...
class Tracker:
        ...
        def L96Command(self,packet_type,packet_data):
                # For info on packet_type and packet_data, refer to L96_GNSS_Protocol_Specification_V1.0
                # packet_type is a string
                # packet_data is also a string, but may contain comma seperated fields
                if not isinstance(packet_type, str):
                        raise TypeError('packet_type is not an instance of str')

                if not isinstance(packet_data, str):
                        raise TypeError('packet_data is not an instance of str')

                
                packet_internal = "PMTK" + packet_type + "," + packet_data
                checksum = self.NMEA_Checksum(packet_internal)
                packet = "$" + packet_internal + "*" + checksum + "\r\n"
                print(packet)
                
                self.L96_SERIAL.wave_clear()
                self.L96_SERIAL.wave_add_serial(L96_SOFT_TX,9600,packet)
                wid = self.L96_SERIAL.wave_create()
                self.L96_SERIAL.wave_send_once(wid)

        def NMEA_Checksum(self,message):
                # get rid of the $ if it's present
                if message[0] == '$':
                        message = message[1:]
                # Make sure that the *00\n\r are not included in our checksum
                if message[-1] == '\r':
                        message = message[:-5]
                checksum = 0
                for byte in message:
                        checksum ^= ord(byte)
                # hex() returns 0x00, so we have to get rid of the 0x
        ...
        def readNMEA(self):
                ...
                if count:
                        fileLog.writelines(data.decode(encoding="utf-8", errors='ignore'))
                        return data
